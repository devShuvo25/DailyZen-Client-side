...
const Navbar = () => {
  ...
  return (
    <div className="sticky top-0 z-50 px-2 sm:px-4 py-1.5 sm:py-2">
      <div className={`navbar rounded-xl sm:rounded-2xl shadow-lg px-3 sm:px-4 md:px-6 h-14 sm:h-16 max-w-7xl mx-auto transition-colors duration-300 ${isDark ? 'bg-slate-800/90 backdrop-blur-md border border-slate-700' : 'glass-effect'}`}>
        {/* LEFT: Logo + Hamburger */}
        <div className="navbar-start flex items-center gap-2 sm:gap-3">
          <button
            className={`lg:hidden p-2 sm:p-2.5 rounded-lg transition-all active:scale-95 ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}
            onClick={() => setIsDrawerOpen(true)}
            aria-label="Open menu"
          >
            <FaBars className={`text-lg sm:text-xl ${isDark ? 'text-slate-300' : 'text-slate-700'}`} />
          </button>

          <Link to="/" className="hidden lg:flex items-center gap-2 group">
            <div className={`p-2 rounded-xl group-hover:rotate-12 transition-transform duration-300 ${isDark ? 'bg-emerald-900/50' : 'bg-emerald-100'}`}>
              <GrWheelchairActive className="text-2xl text-[#10B981]" />
            </div>
            <span className="text-2xl font-extrabold tracking-tight">
              <span className={isDark ? 'text-white' : 'text-slate-800'}>Daily</span>
          ...
          {user ? (
            <Link to="/" className="lg:hidden flex items-center gap-1.5 sm:gap-2">
              ...
              <span className={`text-base sm:text-xl font-bold hidden xs:inline ${isDark ? 'text-white' : 'text-slate-800'}`}>
        ...
        <div className={`navbar-center hidden lg:flex gap-1 items-center p-1.5 rounded-2xl border transition-colors duration-300 ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50/50 border-slate-100'}`}>
          {[
            { path: "/", name: "Home" },
        ...
        {/* RIGHT: Theme Toggle + User or Auth Buttons */}
        <div className="navbar-end gap-2 sm:gap-3">
          {/* Theme Toggle Button */}
          <button
            onClick={toggleTheme}
            className={`p-2.5 sm:p-3 rounded-xl transition-all duration-300 active:scale-95 ${
              isDark 
          ...
          {user ? (
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <Link to="/login" className={`px-3 sm:px-4 lg:px-5 py-1.5 sm:py-2 text-sm sm:text-base font-semibold transition-colors ${isDark ? 'text-slate-300 hover:text-white' : 'text-slate-600 hover:text-slate-900'}`}>
                Login
              </Link>
      ...
      <AnimatePresence>
        {isDrawerOpen && (
          <>
            <motion.div
              className={`fixed inset-0 backdrop-blur-sm z-40 ${isDark ? 'bg-slate-950/60' : 'bg-slate-900/40'}`}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            <motion.div
              className={`fixed top-0 left-0 h-full w-[85%] sm:w-[75%] max-w-[340px] shadow-2xl z-50 flex flex-col p-5 sm:p-6 md:p-8 transition-colors duration-300 ${isDark ? 'bg-slate-800 border-r border-slate-700' : 'bg-white'}`}
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              <div className="flex justify-between items-center mb-8 sm:mb-10">
                <Link to="/" className="flex items-center gap-2" onClick={() => setIsDrawerOpen(false)}>
                  ...
                  <span className={`text-xl sm:text-2xl font-extrabold ${isDark ? 'text-white' : 'text-slate-800'}`}>
                    Daily<span className="text-emerald-500">Zone</span>
                  </span>
                </Link>
                <button
                  className={`p-2.5 rounded-lg transition-all active:scale-95 ${isDark ? 'hover:bg-slate-700' : 'hover:bg-slate-100'}`}
                  onClick={() => setIsDrawerOpen(false)}
                  aria-label="Close menu"
                >
                  <FaTimes className={`text-lg sm:text-xl ${isDark ? 'text-slate-400' : 'text-slate-400'}`} />
                </button>
              </div>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `group relative flex items-center justify-between px-5 sm:px-6 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl text-base sm:text-lg font-black transition-all active:scale-[0.98] ${
                        isActive
                          ? isDark
                            ? "bg-gradient-to-r from-emerald-900/50 to-emerald-800/50 text-emerald-400 pl-7 sm:pl-8 shadow-lg shadow-emerald-500/10"
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        <span className={`opacity-0 group-hover:opacity-100 transition-opacity text-lg sm:text-xl ${isActive ? (isDark ? 'text-emerald-400' : 'text-emerald-400') : (isDark ? 'text-slate-600' : 'text-slate-200')}`}>
                          →
                        </span>
                      </>
                    )}
              ...
              <div className={`mt-auto pt-4 space-y-2.5 sm:space-y-3 border-t ${isDark ? 'border-slate-700' : 'border-slate-100'}`}>
                {user ? (
                  <button
                    onClick={() => {
                      handleLogout();
